$( document ).ready(function(){
    const urlParams = new URLSearchParams(window.location.search);
    const error = urlParams.get('error');
    if(error){
    if(error == 'alreadyindb'){
        var msg = 'Ошибка! Адрес электронной почты уже используется. Попробуйте войти'
    } else if(error == 'passwordisincorrect'){
        var msg = 'Ошибка! Пароли не совпадают '
    } else if(error == 'dataisincorrect'){
        var msg = 'Ошибка! Неверный логин или пароль'
    } else if(error == 'carterror'){
        var msg = 'Ошибка! Авторизуйтесь для добавления товара в корзину'
    }
    $('.template-body').append(`
        <div class='error-message'>
            <svg width="36" height="36" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M11.9999 6.00002V9.60002M11.9999 13.2V13.1098M11.6869 16.5913L6.67816 21.6V16.5913H4.7999C3.47442 16.5913 2.3999 15.5168 2.3999 14.1913V4.80003C2.3999 3.47454 3.47442 2.40002 4.7999 2.40002H19.1999C20.5254 2.40002 21.5999 3.47454 21.5999 4.80002V14.1913C21.5999 15.5168 20.5254 16.5913 19.1999 16.5913H11.6869Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
            <p>${msg}</p>
        </div>
        `)
    }
})